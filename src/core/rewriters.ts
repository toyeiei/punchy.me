...
export class BazukaHandler {
	...
	element(element: Element) {
		...
		if (id === 'card-nickname') {
			element.setInnerContent(escapeHTML(this.data.nickname));
			element.setAttribute('data-text', escapeHTML(this.data.nickname));
		}
		if (id === 'card-job') element.setInnerContent(escapeHTML(this.data.job));
		if (id === 'card-email') element.setInnerContent(escapeHTML(this.data.email));
		if (id === 'card-website') element.setInnerContent(escapeHTML(this.data.website));

		if (id === 'card-email-link') element.setAttribute('href', `mailto:${escapeHTML(this.data.email)}`);
		if (id === 'card-website-link') element.setAttribute('href', escapeHTML(this.data.website));

		if (id === 'title-tag') element.setInnerContent(escapeHTML(`${this.data.nickname} | Digital Business Card | PUNCHY.ME`));
...
export class AnakinHandler {
	...
	element(element: Element) {
		const id = element.getAttribute('id');
		if (id === 'res-name') element.setInnerContent(escapeHTML(this.data.name));
		if (id === 'res-job') element.setInnerContent(escapeHTML(this.data.job));
		if (id === 'res-email') element.setInnerContent(escapeHTML(this.data.email));
		if (id === 'res-website') element.setInnerContent(escapeHTML(this.data.website));

		if (id === 'res-email-link') element.setAttribute('href', `mailto:${escapeHTML(this.data.email)}`);
		if (id === 'res-website-link') element.setAttribute('href', escapeHTML(this.data.website));
		if (id === 'res-summary') {
			if (this.data.aiSummary) {
				element.setInnerContent(escapeHTML(this.data.aiSummary));
		...
		if (id === 'res-experience') {
			if (this.data.aiExperience) {
				element.setInnerContent(escapeHTML(this.data.aiExperience));
				element.removeAttribute('data-pending');
			} else {
				element.setInnerContent(escapeHTML(this.data.experience));
				element.setAttribute('data-pending', 'true');
			}
		}
		if (id === 'res-education') element.setInnerContent(escapeHTML(this.data.education));
		if (id === 'res-skills') {
		...
		if (id === 'title-tag') element.setInnerContent(escapeHTML(`${this.data.name} | Professional Resume | PUNCHY.ME`));
		if (id === 'og-title' || id === 'twitter-title') {
